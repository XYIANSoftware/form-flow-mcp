...
import {
	...
	FormImprovement,
} from '@/lib/mcp/implementations/FormMCP'
...
const ImprovementCard: React.FC<ImprovementCardProps> = ({
	...
	return (
		<Card
			className='improvement-card mb-3 cursor-pointer hover:shadow-md transition-shadow'
			...
			<div className='flex items-start justify-between'>
				<div className='flex-1'>
					<div className='flex items-center gap-2 mb-2'>
						<i
							className={`${getTypeIcon(improvement.type)} ${getTypeColor(
								improvement.type
							)}`}
						/>
				...
				<div className='flex flex-col items-end gap-1'>
					...
					/>
					<i
						className={`${getImpactIcon(improvement.impact)} ${getImpactColor(
							improvement.impact
						)} text-xs`}
					/>
				</div>
...
const QualityDashboard: React.FC<QualityDashboardProps> = ({
	...
	const [qualityAssessment, setQualityAssessment] =
		useState<FormQualityAssessment | null>(null)

	useEffect(() => {
		analyzeQuality()
	}, [form.fields, form.title, form.description, analyzeQuality])
				...
				<>
					...
					<div className='overall-score mb-4'>
						<div className='text-center p-4 bg-gray-800 rounded-lg'>
							...
							/>
							<p
								className={`text-sm font-medium m-0 ${getOverallScoreColor(
									qualityAssessment.overall
								)}`}
							>
					...
					{qualityAssessment.improvements.length > 0 && (
						<div className='improvements'>
							...
							<div className='flex items-center gap-2 mb-3'>
								...
								<Badge
									value={qualityAssessment.improvements.length}
									severity='warning'
									size='small'
								/>
							</div>
					...
					{qualityAssessment.improvements.length === 0 && (
						<div className='text-center p-4'>
							...
							<p className='text-sm text-gray-400 m-0'>
								Your form quality is excellent! No improvements needed.
							</p>
						</div>
					)}
				</>
