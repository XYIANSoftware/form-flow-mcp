...
import { FieldMCP, FieldSuggestion, ValidationSuggestion, FormContext } from '@/lib/mcp/implementations/FieldMCP'
import { FormAssistanceMCP, OptimizationSuggestion } from '@/lib/mcp/implementations/FormAssistanceMCP'
...
const SuggestionCard: React.FC<SuggestionCardProps> = ({ suggestion, onSelect }) => {
	...
	return (
		<Card className="suggestion-card mb-3 cursor-pointer hover:shadow-md transition-shadow" onClick={() => onSelect(suggestion)}>
			<div className="flex items-start justify-between">
				<div className="flex-1">
					<div className="flex items-center gap-2 mb-2">
						<h6 className="text-sm font-medium text-white m-0">{suggestion.title}</h6>
						<Badge 
							value={`${Math.round(suggestion.confidence * 100)}%`}
							severity={suggestion.confidence >= 0.8 ? 'success' : suggestion.confidence >= 0.6 ? 'warning' : 'danger'}
							size="small"
						/>
					</div>
					...
					{suggestion.field && (
						<div className="bg-gray-800 rounded p-2 mb-2">
							...
							<div className="flex items-center gap-2">
								<Badge value={suggestion.field.type} severity="info" size="small" />
								<span className="text-xs text-white">{suggestion.field.label}</span>
					...
					{suggestion.validation && (
						<div className="bg-gray-800 rounded p-2 mb-2">
							...
							<div className="text-xs text-white">{suggestion.validation.rule}</div>
							<div className="text-xs text-gray-400">{suggestion.validation.message}</div>
						</div>
					)}
					...
					{suggestion.grouping && (
						<div className="bg-gray-800 rounded p-2 mb-2">
							...
							<div className="text-xs text-white">{suggestion.grouping.groupName}</div>
							<div className="text-xs text-gray-400">{suggestion.grouping.reason}</div>
						</div>
					)}
				</div>
				...
				<div className="flex flex-col items-end gap-1">
					...
					<i className={`${getImpactIcon(suggestion.impact)} ${getImpactColor(suggestion.impact)} text-xs`} />
				</div>
			</div>
...
const SmartSuggestionsPanel: React.FC<SmartSuggestionsPanelProps> = ({
	...
	useEffect(() => {
		loadSuggestions()
	}, [context.fields, context.purpose])
	...
	const loadSuggestions = async () => {
		...
		try {
			...
			const fieldResult = await FieldMCP.suggestNextFields(context.fields, context)
			...
			if (context.fields.length > 0) {
				...
				const optimizationResult = await FormAssistanceMCP.suggestFormOptimizations(mockForm)
	...
	return (
		<Card className={`smart-suggestions-panel ${className}`}>
			...
			{fieldSuggestions.length > 0 && (
				<div className="mb-4">
					<div className="flex items-center gap-2 mb-3">
						...
						<h6 className="text-sm font-medium text-white m-0">Field Suggestions</h6>
						<Badge value={fieldSuggestions.length} severity="info" size="small" />
					</div>
			...
			{optimizationSuggestions.length > 0 && (
				<div className="mb-4">
					...
					<div className="flex items-center gap-2 mb-3">
						...
						<h6 className="text-sm font-medium text-white m-0">Optimization Suggestions</h6>
						<Badge value={optimizationSuggestions.length} severity="warning" size="small" />
					</div>
					<div className="optimization-suggestions">
						{optimizationSuggestions.map((suggestion, index) => (
							<Card key={`optimization-${index}`} className="optimization-card mb-3">
								<div className="flex items-start justify-between">
									<div className="flex-1">
										<div className="flex items-center gap-2 mb-2">
											<h6 className="text-sm font-medium text-white m-0">{suggestion.title}</h6>
											<Badge 
												...
												severity={suggestion.impact === 'high' ? 'danger' : suggestion.impact === 'medium' ? 'warning' : 'success'}
												size="small"
											/>
										</div>
									...
									<div className="flex flex-col items-end gap-1">
										<Badge 
											...
											severity={suggestion.effort === 'low' ? 'success' : suggestion.effort === 'medium' ? 'warning' : 'danger'}
											size="small"
										/>
									</div>
								</div>
								<div className="flex justify-end mt-3">
									<Button
										...
										className="p-button-sm p-button-outlined"
										onClick={() => {
											console.log('Optimization suggestion applied:', suggestion)
			...
			{/* No Suggestions */}
			{fieldSuggestions.length === 0 && optimizationSuggestions.length === 0 && !loading && (
				<div className="text-center p-4">
					<i className="pi pi-check-circle text-green-500 text-2xl mb-2" />
					<p className="text-sm text-gray-400 m-0">Your form looks great! No suggestions at this time.</p>
				</div>
			)}
